...
const handleList = () => {
  ...
  httpGet(`douson/report/list`, state.query.data).then(
      (res: any) => {
...
const handleMerge = () => {
  formRef.value.validate((valid: any) => {
    ...
    if (valid) {
      if (state.formSave) {
        httpPostJson('douson/report', state.formData).then(() => {
...
const handleUpdate = (row: any) => {
  return httpPutJson('douson/report', row).then(() => {
    state.formVisible = false
