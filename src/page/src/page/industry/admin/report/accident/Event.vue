<template>
  <div>
    <div class="query-container">
      <el-input v-model="query.data.eventId" @keyup.enter="handlePage" :placeholder="store.state.label.accidentEvent" clearable @change="handlePage" class="search-item"/>
      <el-date-picker
      ...
      <el-select v-model="query.data.reason"
                 ...
                 allow-create
                 clearable
                 :placeholder="store.state.label.eventReason"
                 class="search-item">
        <el-option
            v-for="item in config.eventReasonList"
            :key="item.value"
            :label="item.label"
            :value="item.value"
        />
    ...
    >
      ...
      >
        ...
        <el-form-item prop="accidentDescribe" :label="store.state.label.accidentEventDescribe">
          <el-input type="textarea" :rows=4 v-model="formData.accidentDescribe"/>
        </el-form-item>
        <el-form-item prop="photoList" :label="`${store.state.label.eventPhoto}(${(formRuleList['photoList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
        ...
        <el-form-item prop="fileList" :label="`${store.state.label.eventFile}(${(formRuleList['fileList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
          <file-upload :fileList="formData.fileList" :maxSize="Number(`${(formRuleList['fileList'] || []).reduce((p:any, t:any) => t.max, 999)}`)"></file-upload>
        </el-form-item>
        <el-form-item prop="reason" :label="store.state.label.eventReason">
        ...
        <el-form-item prop="solve" :label="store.state.label.eventSolve">
          <el-input type="textarea" :rows=4 v-model="formData.solve"/>
        </el-form-item>
        <el-form-item prop="improvePhotoList" :label="`${store.state.label.improveEventPhoto}(${(formRuleList['improvePhotoList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
        ...
        <el-form-item prop="improveFileList" :label="`${store.state.label.improveEventFile}(${(formRuleList['improveFileList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
        ...
        <el-form-item prop="opinion" :label="store.state.label.eventOpinion">
...
const columnConfigList = ref<ViewConfig[]>([
  ...
  {value: 'accidentDescribe', labelKey: 'accidentEventDescribe', width: 276, showOverflow: true,},
  {value: 'reasonFormat', labelKey: 'eventReason', width: 276, showOverflow: true,},
  {value: 'solve', labelKey: 'eventSolve', width: 276, showOverflow: true,},
...
const handleShowPrintDetail = (d: any) => {
  window.open(`/industry/public/event?eventId=${d.param.eventId}`)
}
...
const handlePage = () => {
  ...
  httpGet(`douson/admin/event/page`, state.query).then(
...
const handleMerge = () => {
  formRef.value.validate((valid: any, fields: any) => {
    if (valid) {
      if (state.formSave) {
        httpPostJson('douson/admin/event', state.formData).then(() => {
...
const handleUpdate = (row: any) => {
  return httpPutJson('douson/admin/event', row).then(() => {
    state.formVisible = false
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('Confirm Delete?', 'Tips', {
    ...
    httpDelete('douson/admin/event', {eventId: row.eventId})
    .then(() => {
