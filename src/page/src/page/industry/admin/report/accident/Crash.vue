<template>
  <div>
    <div class="query-container">
      <el-input v-model="query.data.crashId" @keyup.enter="handlePage"
                :placeholder="store.state.label.accidentCrash" clearable @change="handlePage" class="search-item"/>
      <el-date-picker
...
const handleShowPrintDetail = (d: any) => {
  window.open(`/industry/public/crash?crashId=${d.param.crashId}`)
}
...
const handlePage = () => {
  ...
  httpGet(`douson/admin/crash/page`, state.query).then(
...
const handleMerge = () => {
  formRef.value.validate((valid: any, fields: any) => {
    if (valid) {
      if (state.formSave) {
        httpPostJson('douson/admin/crash', state.formData).then(() => {
...
const handleDelete = (row: any) => {
  ElMessageBox.confirm('Confirm Delete?', 'Tips', {
    ...
    httpDelete('douson/admin/crash', {crashId: row.crashId})
    .then(() => {
