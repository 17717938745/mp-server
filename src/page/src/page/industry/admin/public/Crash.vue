  ...
  <div style="display: flex; flex-direction: column; align-items: center; justify-content: flex-start; margin-bottom: 20px; position: absolute; width: 100%; top: 20px;" id="printContainer">
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 595px;">
      ...
      >`
        <el-descriptions-item
            ...
            align="center"
            label-class-name="crash-print-label description-print-border"
    ...
    >
      ...
      >
        ...
        <el-form-item prop="reason" :label="store.state.label.crashReason">
          <el-select v-model="data.reasonList"
                     filterable
                     multiple
                     allow-create
                     clearable
                     :placeholder="store.state.label.crashReason"
                     class="search-item">
            <el-option
                v-for="item in config.crashReasonList"
                :key="item.value"
                :label="item.label"
                :value="item.value"
            />
...
const handleGet = () => {
  httpGet(`douson/crash`, {crashId: route.query.crashId}).then(
      (res: DataResult<any>) => {
        data.value = res.data || {}
...
const handleUpdate = (row: any) => {
  return httpPutJson('douson/admin/crash', row).then(() => {
...
const handleCopyLink = () => {
  const text = dousonFullUrl(`/industry/public/crash?crashId=${route.query.crashId}`)
