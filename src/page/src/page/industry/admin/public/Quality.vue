  ...
  <div style="display: flex; flex-direction: column; align-items: center; justify-content: flex-start; margin-bottom: 20px; position: absolute; width: 100%; top: 20px;" id="printContainer">
    <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 595px;">
      ...
      >`
        <el-descriptions-item
            ...
            align="center"
            label-class-name="quality-print-label description-print-border"
    ...
    >
      ...
      >
        ...
        <el-form-item prop="fileList" :label="`${store.state.label.qualityFile}(${(formRuleList['fileList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
        ...
        <el-form-item prop="improvePhotoList" :label="`${store.state.label.improveQualityPhoto}(${(formRuleList['improvePhotoList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
        ...
        <el-form-item prop="improveFileList" :label="`${store.state.label.improveQualityFile}(${(formRuleList['improveFileList'] || []).reduce((p:any, t:any) => `Min: ${t.min}, Max: ${t.max}`, 'Unlimited')})`">
...
const handleGet = () => {
  httpGet(`douson/quality`, {qualityId: route.query.qualityId}).then(
      (res: DataResult<any>) => {
        data.value = res.data || {}
...
const handleCopyLink = () => {
  const text = dousonFullUrl(`/industry/public/quality?qualityId=${route.query.qualityId}`)
