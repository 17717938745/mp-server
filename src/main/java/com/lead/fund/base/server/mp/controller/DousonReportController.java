    ...
    public ListResult<ReportSummaryAccountResponse> reportSummaryAccount(
        ...
        list.stream()
                .collect(Collectors.reducing(
                    ...
                    list.add(t
                            // 在平均
                            .setDeviceCompletePercent(t.getDeviceCompletePercent().divide(new BigDecimal(atomicInteger.get()), 8, RoundingMode.HALF_UP))
                            .setIndex(atomicInteger.addAndGet(1))
                    );
                });
    ...
    public ListResult<ReportSummaryDeviceResponse> reportSummaryDevice(
        ...
        final List<ReportSummaryDeviceResponse> list = rl.stream()
                .map(t -> new ReportSummaryDeviceResponse()
                        ...
                        .setUserIdList(CollUtil.newHashSet(t.getUserId()))
                        .setReportDateList(CollUtil.newHashSet(t.getReportDate()))
                        .setReportDateCount(1)
                        .setTotalCount(1)
        ...
        list.stream()
                .collect(Collectors.reducing(
                    ...
                    // 在平均
                    list.add(t
                            .setDeviceUsePercent(t.getDeviceUsePercent().divide(new BigDecimal(atomicInteger.get()), 8, RoundingMode.HALF_UP))
                            .setDeviceCompletePercent(t.getDeviceCompletePercent().divide(new BigDecimal(atomicInteger.get()), 8, RoundingMode.HALF_UP))
                            .setIndex(atomicInteger.addAndGet(1))
                    );
                });
